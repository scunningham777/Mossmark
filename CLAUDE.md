...
- **`ConversionDef` ScriptableObject (Iteration 36, `Mossmark.Inventory`)**: each `Input` entry has a `Kind` enum (`Item` or `Property`). Item-keyed inputs match a specific `ItemDefinition` + `quantity`; property-keyed inputs match any item carrying the named `propertyId`. `TryMatch(ItemDefinition[] placed, out int[] matchedSlots)` does greedy left-to-right matching — copies placed[] to a mutable remaining[], nulls matched entries to prevent double-use, returns false on first unsatisfied input. `matchedSlots[i]` is the last matched slot index for input `i`. Three live recipes: `recipe_plaited_cord` (3×Reeds → Plaited Cord), `recipe_tinder_bundle` (2×Bark Strips + 1×Sticks → Tinder Bundle), `recipe_weighted_snare` (1×binds_fast + 1×heavy_true → Weighted Snare).
- **`WorkshopAttendable` (Iteration 36)**: `DevelopableEntity, IAttendable` — does NOT inherit `BuildingAttendable` (single-stage restoration kept self-contained to avoid duplicating multi-stage complexity). One `DevelopmentStage` ("workshop_restoration") built in `Awake()`. `CanAttend()` returns true if restored OR `CanMakeProgress()`. When restored, `OnAttentionComplete()` sets `lastAttentionWasOpen = true` and calls `WorkshopUI.Instance.Open(this)` — `RequiresDaylight` and `ContinueAttending` both read `!lastAttentionWasOpen` so opening the UI costs no daylight and doesn't continue the attention loop. No `IMaintenanceConsumer` — intentional, keep the workshop simple for Iteration 36.
- **`WorkshopUI` slots are references, not removals (Iteration 36)**: placing an item into a WorkshopUI slot does NOT remove it from inventory. Items are consumed only at Work time on success (`HandleSuccess()`). This eliminates any need for a return-all-items path on Close(). `HandleSuccess()`: consumes all matched inputs via `InventoryManager.RemoveItem()`, calls `PropertyKnowledge.MarkKnown()` for every property on every consumed item, adds output item, posts flavor text, calls `EntityFeedback.TriggerPop()` on the workshop GO. `HandleFailure()`: finds all `(item, propId)` pairs across placed items where `!PropertyKnowledge.IsKnown()`, picks one at random, marks it known, posts a notification — if all properties already known, posts `"nothing comes of it."` Each Work attempt costs 1 daylight via `DayCycleManager.SpendDaylight()` regardless of outcome.
