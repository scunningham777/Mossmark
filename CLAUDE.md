...
| Property-Phrased Wants: Loop Closure (Iteration 37) | Complete | `PropertyAvailableCondition` (carry-only property gate, authored `wantDescription` shown in overlay), `NpcPostSpecStageDef.requiredPropertyId`/`wantDescription` fields, Bog Keeper `bog_keeper_drainage` stage gates on `turns_water` (Mistletoe or Clay satisfy it), matched item consumed from pack at stage completion |
...
- **`PropertyAvailableCondition` (Iteration 37, `Mossmark.Development`)**: implements `IDependencyCondition` — checks `InventoryManager.Instance.Stacks` (carry-only, not chest) for any item whose `PropertyIds` contains the required `propertyId`. `GetNeedsDescription()` returns the authored `wantDescription` field (falls back to `"needs something that {phrase}"` via `PropertyRegistry.GetById()` if empty). `NpcPostSpecStageDef` gains two new fields: `requiredPropertyId` (non-empty = property gate instead of item gate) and `wantDescription`. `NpcAttendable.BuildPostSpecStages()` branches: property-gated stages use `PropertyAvailableCondition`; item-gated stages keep the existing `ItemAvailableCondition` path. When a property-gated stage fires, `NpcAttendable.ConsumePropertyMatchedItem(propertyId)` removes one item from carry — picks the stack with the highest quantity (most common). Live instance: `bog_keeper_drainage` (Bog Keeper) gates on `turns_water`; overlay reads "needs something that turns water for the drainage channels." `bog_keeper_iron_sense` remains exact-item for contrast. Content path verified: both Mistletoe and Clay carry `turns_water` and are discoverable at the Workshop before Bog Keeper specializes.
