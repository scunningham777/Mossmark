...
| `Mossmark.Prototype4` | Acquaintance pilot scripts scoped to `Prototype4.unity` (`AcquaintableAttendable`, `AttentionCountCondition`, `TaughtPropertyCondition`, `TendableSpotAttendable`, `EarnedSurfaceAttendable`, `WreckAttendable`) — new components only, same Reuse Discipline as P3; nothing in Greybox or Prototype3 references this namespace |
...
| P4 Site Character: Single-Site Pilot (Iteration 4.15) | Complete (partial live verification) | `TendableSpotAttendable.character` — persistent, never-resolving float nudged by the same `daysAway`/`attendsToday` state ripeness already tracks (a ripe attend nudges up, a same-day-depleted attend nudges down harder), deliberately kept off `yieldChance`/ripeness itself; two outlets, `RefreshTint()` (continuous `Color.Lerp`) and `CurrentFlavorPool()` (banded miss-line selection, fires even after the item pool is exhausted); The Landing is the authored pilot site, The Ash Bed inherits the mechanism with generic content only |
| P4 One Dependency Edge: Cross-Site Pilot (Iteration 4.16) | Complete (mechanism verified; felt-pull playtest pending) | Zero code changes — a fourth `AcquaintanceStage` on the Collier (`banked_to_keep`), gated on `TaughtPropertyCondition("keeps_well")`, the property currently discoverable only via the Netmender/Smoking Racks thread; confirms `TaughtPropertyCondition`'s existing "does the player generally know this" check composes across sites with no new gating logic — the Collier's ladder is genuinely bottlenecked on a fact learned at a different site |
| P4 War Scars: Recovery Lean (Iteration 4.17) | Complete (revised 7-24-26; felt-legibility playtest still pending) | `WreckAttendable` (`Mossmark.Prototype4`, plain `MonoBehaviour : IAttendable`, no `DevelopableEntity`) — The Wrecked Cart, a single hand-placed POI whose `recoveryLean` float resolves once into *Salvaged* or *Repaired* with no player-facing choice ever presented; revised from a per-attend nudge (which conflated "checking if more's available" with "choosing to linger," since both produced the same tick) to a day-resolved nudge — `HandleDayAdvanced` reads that day's total attend count once, at rest, and nudges continuously off `neutralAttendsPerDay`, decoupling the lean from RNG entirely; no tint/flavor tell before resolution, permanent lock after |
