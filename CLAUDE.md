...
| P4 War Scars: Recovery Lean (Iteration 4.17) | Complete (mechanism verified; felt-legibility playtest pending) | `WreckAttendable` (`Mossmark.Prototype4`, plain `MonoBehaviour : IAttendable`, no `DevelopableEntity`) — The Wrecked Cart, a single hand-placed POI whose `recoveryLean` float resolves once into *Salvaged* or *Repaired* with no player-facing choice ever presented; a day-capped single roll against a 2-item pool nudges down on a claim, and any attend past that day's roll (or once the pool's claimed) nudges up, so "grab and go" vs. "keep holding" reads purely from ordinary hold behavior; no tint/flavor tell before resolution, permanent lock after |
