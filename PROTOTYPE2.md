...
- **Document split — post-prototype feature doc**: PROTOTYPE2.md is approaching the size where implementation history becomes noise in the context window. The planned split: once Iterations 29 and 30 are complete, a new `FEATURES.md` begins for post-prototype work (wilderness tendedness long-game, ritual manipulation view, full maintenance economy, population dynamics, inter-region travel). PROTOTYPE2.md is retained as a historical record of the prototype arc but stops receiving new iteration entries. CLAUDE.md's Project Goal section will be updated to point to the new doc as the active roadmap.
