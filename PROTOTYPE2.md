...
Iteration 8.5 is inserted between 8 and 9 (rather than renumbering 9-17) so existing "Iteration N" references in CLAUDE.md's history stay accurate.
...
| 17 | [ ] | Playtest + Tuning Pass | Full day loop playable end-to-end; daylight costs, yield tables, and dependency thresholds tuned for a legible web | — |
...
- **Specialized NPC's own further advancement**: once an NPC specializes, does it have its own development track beyond the initial specialization choice? Still unresolved — `NpcAttendable` (Iteration 10) becomes inert (`CanAttend() => false`) once `CurrentStageIndex >= 0`, deferring this question rather than answering it.
