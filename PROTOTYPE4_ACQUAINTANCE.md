...
# Iteration 4.18 — Depth Has a Price: Gating Known on a Prerequisite (Single-Entity Pilot)

## Where this comes from

Scoped 7-24-26. The oldest open item in *Next Steps* (first flagged 7-18-26, re-flagged at 4.13): there's no real tension yet between diving deep on one entity vs. spreading across several, because right now nothing distinguishes the *cost* of reaching Acquainted from the cost of reaching Known beyond "a couple more attends, same sitting." Sean's steer: it should be cheap to get a surface read on two or three things in a day, but reaching Known for any one of them should cost meaningfully more than that — multi-day attending, a taught property, or some other real commitment — and whatever the mechanism, it should be built from content already obtainable within the two existing sites, so the *shape* of the dependency exists now and can be retargeted to require travel between sites once more sites exist.

## The claim under test

Does gating Known on an already-known property — rather than on attends alone — make the jump from Acquainted to Known feel like a real commitment, distinct in weight from the cheap surface read Acquainted already gives? And does hitting that gate before the prerequisite is met create a legible pull toward developing something else, rather than reading as an arbitrary wall?

**This pilot is deliberately single-entity, not scene-wide.** The other five entities keep their current shape untouched — they're the cheap-breadth baseline the gated entity needs to feel *asymmetric against*, not something this iteration also has to prove.

## Mechanism

**No new condition type.** `TaughtPropertyCondition` (4.10/4.11) already gates a stage on `WorldContext.IsPropertyKnown("p3_player", propertyId)`, checked without regard to where the property was learned. Today it's only ever authored on a stage sitting *past* Known (`smokes_the_catch`, `shares_the_watch`). This iteration retargets it one slot earlier — onto Known itself — for one entity only. Purely a content/authoring change, same shape 4.16 already confirmed for cross-site interlock ("nothing about the pipeline needs to change to make one site's gate reference a property... this pilot is a content/authoring change, not a systems change").

**Entity: the Netmender.** Chosen over any other candidate because she already carries a proven rejection texture for this exact situation (4.11's `earlyTeachHintLine`, "not ready to hear that yet") and already sits downstream of a real, already-built same-site chain: `binds_fast` is revealed only via the Smoking Racks, which only open once the Osier Bed itself reaches Known (4.9). Nothing about that chain needs to be built — it already exists; this iteration just makes the Netmender's own Known stage consume it.

**Her ladder, before and after:**
- Before: `Unfamiliar → Acquainted → Known → [taught: keeps_well] Smokes the Catch`
- After: `Unfamiliar → Acquainted (unchanged) → Known [taught: binds_fast] → [taught: keeps_well] Smokes the Catch`

Acquainted is untouched — same `minAttends`/`ripenChance` tuning as every other entity (2/0.5), still reachable in a single sitting. Known now requires `binds_fast` already known before it can ripen at all; the existing `InOrderCondition` (4.7) already enforces that this doesn't skip ahead of Acquainted. The post-Known `smokes_the_catch` stage is untouched — she now carries a genuine two-deep prerequisite chain (`binds_fast` then `keeps_well`), both reusing existing condition machinery, no new stage type.

**The hinge from 4.10 (`IsFullyAcquainted` reading `NextStageDef == null || !string.IsNullOrEmpty(NextStageDef.taughtPropertyId)`) needs no change** — it was built to handle exactly this shape (a taught-gated stage as "next"), regardless of which array position that stage sits at.

## Explicitly out of scope

- **Any change to the other five entities.** Smokehouse, Osier Bed, Collier, Bothy, Hearth Ring keep their current all-attends ladders — they're the comparison baseline, not part of what's under test.
- **Any day/timer gate.** Deliberately not layered in alongside the property gate — if a property gate alone doesn't produce a felt weight difference, a multi-day requirement is the next thing to try in a separate pilot, not folded in now to hedge.
- **Any cross-site requirement.** `binds_fast`'s source (Osier Bed → Racks) stays inside The Landing, per Sean's explicit scoping. Swapping the source to the Ash Bed/Hearth Ring site is the natural next generalization once this lands — not part of this pilot.
- **Any change to `TaughtPropertyCondition` itself**, the teach pipeline, or the `InOrderCondition` sequencing — pure retargeting of existing, proven machinery.

## Success criterion (playtest, not metric)

Starting from a state where the Netmender is Acquainted but `binds_fast` isn't yet known: does attending her (attempting to reach Known) surface the existing rejection texture and create a real pull toward the Osier Bed — "I need to go develop that before I can go further here" — rather than reading as an arbitrary wall? And once `binds_fast` is earned and the player returns, does crossing into Known feel like a bigger, more committed moment than Acquainted did — genuinely different in weight, not Acquainted-plus-an-errand?

If the two stages still feel like the same kind of moment with an extra step tacked on, that's the real finding: a property gate alone isn't enough asymmetry, and a day/commitment-based gate is likely the piece actually missing — not a reason to add more properties to the chain.

## Why this is the right next pilot

It's nearly free — one stage's condition retargeted on one already-well-understood entity, zero new code, reusing a rejection texture and a dependency chain that are both already built and already playtested individually. It isolates exactly one new claim (property-gated Known vs. attend-gated Known) from every other variable, with the rest of the site sitting untouched in the same scene as a live depth-vs-breadth comparison. If it lands, generalizing to the other entities — and later, swapping same-site sources for cross-site ones — is informed rather than guessed at.

---

## Iteration 4.18 Build Notes (7-24-26)

Built and verified live in Play Mode via MCP. **The Greybox + Prototype3 regression gate was skipped this session at Sean's request, same as 4.17's precedent** — he's running it himself. New code confined to `Mossmark.Prototype4.AcquaintableAttendable`; no shared script touched. The scene change is content-only on the Netmender (`Prototype4.unity`), hand-edited YAML per the standing MCP-Unity object-reference limitation (`AcquaintableAttendable` carries no `UnityEngine.Object` references, same precedent as every prior hand-YAML pass in this doc).

**A real hinge bug found before verification, not just the retargeting the doc anticipated:** the doc's mechanism section claimed "the hinge from 4.10 needs no change," but authoring a taught gate on Known itself — one slot earlier than every prior taught stage, which had only ever sat *past* the ladder's own end — breaks that assumption in two connected ways:

1. `IsFullyAcquainted` (`NextStageDef == null || NextStageDef.taughtPropertyId set`) now goes true the moment the player reaches Acquainted, not Known, since Known is NextStageDef and is now taught-gated. That part is actually correct under the formula's own stated meaning ("nothing further reachable by pure attention") — Known genuinely isn't attention-reachable anymore. But `GetOverlayDescription()`'s branch order checked `IsFullyAcquainted` *before* `EarlyTeachAttemptPending`, and `EarlyTeachAttemptPending` itself bailed out whenever `IsFullyAcquainted` was true — so the moment the property gate became live (exactly the state the success criterion needs to surface a rejection texture in), the hint mechanism was structurally unreachable, dead code for this exact case.
2. Separately, `TaughtStageDef` (4.11) always resolved to the array's *final* entry — a quirk 4.16's build notes already flagged once the Collier picked up a second taught stage, but never fixed since nothing depended on the fix yet. With Known now taught-gated ahead of `smokes_the_catch`, "always the last entry" would have pointed the hint at the wrong (further-off) gate entirely.

Fixed both at once: `TaughtStageDef` became `NextTaughtStageDef`, a forward scan from `CurrentStageIndex + 1` for the nearest stage with a `taughtPropertyId` set (correct for both an immediate gate like Known and a distant one like the Collier's original pre-Known preview). `EarlyTeachAttemptPending`'s guard changed from `if (IsFullyAcquainted) return false` to `return !TeachPending` — the actually-correct "not ready yet" signal is whether teaching is live right now, not whether pure attention has run out. `GetOverlayDescription()`'s branch order changed to check `EarlyTeachAttemptPending` before `IsFullyAcquainted` (previously order didn't matter, since the two were mutually exclusive by construction; now it does). `SpentToday` kept first priority throughout, unchanged.

**A deliberate, and real, behavior change to the Collier (4.11/4.16), flagged rather than smoothed over:** the old `EarlyTeachAttemptPending` only fired once the player had *already* discovered the paired property (`WorldContext.IsPropertyKnown(playerKnowerId, ...)` was the final gate). The new version drops that requirement — it fires whenever a taught stage is next and the entity hasn't been taught it, whether or not the player has found the property yet. This was necessary for 4.18's own success criterion, which explicitly starts from a state where the player *hasn't* found `binds_fast` yet and still expects a felt rejection. Since every authored `earlyTeachHintLine` in the scene (the Collier's, and the Netmender's new one) is deliberately vague and never names the gated property, showing it earlier doesn't leak anything — it reads the same as the dependency resolver's own "needs" message style (legible that something's missing, not what). Net effect: the Collier's hint now also surfaces before the player has found `heavy_true` or `keeps_well`, not just after. Not re-verified against 4.11's exact original transcript beyond the cold-load spot check below (hint present, unchanged text) — flagged here in case a future session cares about the distinction.

**The content:** Netmender's `seededPropertyIds` emptied (`binds_fast` was previously self-known from scene load — the exact fact this iteration needed to gate on being taught instead, so keeping it seeded would have made `TaughtPropertyCondition` trivially satisfied from `Awake()` and the whole gate a no-op). Known's `taughtPropertyId` set to `binds_fast`, tuning changed from the old attends-only `2/0.2` to `1/0.5` — the same generous once-earned tier every other taught stage in this doc already uses ("once earned, the payoff shouldn't make the player wait long"), since Known is now that kind of stage. New `earlyTeachHintLine` ("She works past you without slowing. Whatever you know of binding, you haven't shown her yet — and she only opens up to someone who has.") and `teachLine` ("She takes the withy from your hand, bends it near double, lets it spring back straight. \"Binds fast, this. You've been to the bed, then.\" Something in how she says it settles — like you've paid a toll she didn't ask for, but noticed all the same."). Acquainted and `smokes_the_catch` untouched.

**Verified live (MCP, Play Mode, 7-24-26), one continuous real-path run:**
1. Cold load, 0 errors — Netmender Unfamiliar, `knows=[]` (confirms the seeded-property fix), overlay already carrying the hint ("...she only opens up to someone who has") even at Unfamiliar — same "shows early" texture as the Collier's, expected under the new `!TeachPending` gating.
2. Debug-advanced to Acquainted. Overlay still carries the hint; interaction line still the ordinary "Hold E to spend a while with them" (not the teach line — `TeachPending` correctly false, player doesn't know `binds_fast` yet).
3. **The wall, confirmed with a real held attend**: one real Begin/Release Attend at Acquainted, `binds_fast` still unknown to the player. Result: `acquaintanceStage` stayed at `0` — no ripening attempt, no error, just an ordinary flavor visit. `CanMakeProgress()` correctly read false against Known's now-live `TaughtPropertyCondition`.
4. **The real discovery path, no shortcuts for the property itself**: Osier Bed and Smokehouse debug-advanced to Known (unrelated ladders, not what's under test here — same time-saving precedent 4.16 used); a real tending hold at The Landing yielded *A Cut Withy* (several days and misses first — `TendableSpotAttendable`'s RNG, unrelated to this iteration); two real held attends at the Smoking Racks revealed `burns_slow` on the Alder Billet then `binds_fast` on the Withy, the latter marked under `p3_player` exactly as `TaughtPropertyCondition` reads.
5. **Back at the Netmender, unprompted**: `Log Entity State` showed the interaction line already flipped to `"Hold E to speak of what binds fast"` and the hint gone from the description — before any debug tool touched her, `TeachPending` picked up the real discovery path exactly as it does for every other taught pairing in this doc.
6. A real held attend resolved as the teach tick: `knows=[binds_fast]` on the entity itself immediately after, and the description grew *in the same tick* to `"...They nod when you come up the path now. They speak of what binds fast."` — still at `acquaintanceStage=0` (teaching doesn't itself cross the stage, same as `smokes_the_catch`/`shares_the_watch` before it), interaction line reverted to the ordinary Acquainted line.
7. Two real ripening attempts at Known's `1/0.5` tuning both missed (expected variance at a coin-flip chance); `Advance Acquaintance On Nearest Entity` forced the third across: `acquaintanceStage=1`, tint `RGBA(0.45, 0.62, 0.4, 1)` (Known's authored tint), full Known description plus the grown seeded-knowledge line, interaction `"Hold E to pass a while with them"` — text identical to the pre-4.18 Known read, confirming the destination itself wasn't changed, only the path to it.
8. 0 console errors, 0 warnings across the entire session (cold load through the final cross).

**What this session did and didn't answer:** the mechanism is now confirmed working end-to-end — the wall holds before the property is known, the interaction line and hint correctly track `TeachPending` state, teaching lands and immediately reads back, and the crossing lands on the same authored Known content as before. What it didn't answer is the iteration's actual success criterion — whether the wall creates a *felt* pull toward the Osier Bed rather than reading as arbitrary, and whether Known now lands as a bigger, more committed moment than Acquainted. That's a human-playtest question, the same distinction this doc has drawn before for felt/tone judgments (4.15, 4.16, 4.17). Recorded here as mechanism-verified and hinge-fixed, not yet playtested.

---

## Playtest Findings (Sean, 7-26-26)

The multi-stage shape of 4.18 landed well — liked that Known now costs more than Acquainted. But playing the actual `binds_fast` chain (get to Known via the Osier Bed → Racks → teach path) surfaced tending at The Landing sliding back toward the vending-machine feel P2 already broke from once: fighting RNG for a specific, already-decided-on item, with "wait a few days" as the only lever — and in the moment, that lever read as a *distraction* from the goal rather than a legitimate parallel task.

**A concrete bug-shaped piece of it, not just tone:** `character` (4.15) nudges per-attend and never resets; ripeness (4.13) resets/recovers per day. The two axes can disagree — attended The Landing first thing one morning, ripeness fresh and odds should have read favorable, and got the "picked over" flavor line anyway, because `character` was still low from the previous day's depletion and hadn't had a chance to recover on its own slower clock. Reads as the game chastising the player for a visit it hasn't made yet that day. Candidate fix, same shape as 4.17's per-attend → day-resolved revision: assess/nudge `character` once at rest (`DayCycleManager.DayAdvanced`) instead of per-attend, so it can't contradict a same-morning ripeness bonus. Not built this session — flagged for a later pass.

**The deeper question, still open:** why the RNG reads as frustrating here specifically, when the same mechanism (probabilistic yield) works fine elsewhere in P4 (acquaintance ripening, undirected tending with no specific target). Working hypothesis, not yet tested: it's not that the player doesn't know where to go (in this case, not knowing *which* of two tending spots to check actually reduced frustration, by giving a legitimate second task to fall back on) — it's that a miss reads differently depending on whether the player is exploring or executing. Against an undecided goal, a miss is "not yet, keep looking" (Organic, as intended). Against an already-decided goal — 4.18 made `binds_fast` a hard prerequisite for Known — the same miss reads as "no," because there's no alternate path to that *specific* goal, only "try again" or "abandon this and do something else," and "something else" isn't a real choice when the something-else isn't what the player actually wants right now. 4.18 also stacks two probabilistic layers on one goal-directed path (tending's yield roll, then 4.7's ripening roll on the teach itself) that were each fine alone. Not concluded — a candidate diagnosis to test against, not a decided finding.

---

# Iteration 4.19 — A Second Front (Two-Entity Pilot)

## Where this comes from

Same 7-26-26 conversation as the Playtest Findings above, continued. Daylight was bumped 6 → 7 that session as a quick, un-iterated tweak (Sean's own call, not scoped here) and felt less tedious — kept, but orthogonal to what follows and not re-litigated.

The open question from the RNG-frustration hypothesis: 4.18 was deliberately a *single-entity* pilot — the other five entities are "cheap breadth," but cheap breadth isn't the same as a real second commitment. While blocked on the Netmender's `binds_fast` gate, there was nothing else at the same weight to turn to; foraging at the other tending spot or attending an un-gated entity doesn't feel like *progress on something that matters*, it feels like filler. Sean's read: that scope-narrowing may be clouding the diagnosis — worth testing whether a genuine second commitment, blockable and workable independently of the first, changes the felt experience before concluding anything about the gate mechanism itself.

## The claim under test

Does having a second, independently-sourced hard commitment — something the player can make real, decided progress on while blocked elsewhere — change how the RNG/day-gated wait on the *first* commitment feels? If frustration drops once there's a genuine B to work while A is blocked, that confirms 4.18's read was partly an artifact of the single-entity pilot's own scope, not a property of the property-gated-Known mechanism. If it doesn't drop, the 7-26 diagnosis (compounding RNG layers on an already-decided goal) holds regardless of how much content exists, and the fix belongs in the tending/ripening mechanism itself — not in adding more parallel content.

**Deliberately not fixing the `character`/rest-timing bug (also flagged 7-26) as part of this pilot.** If both land in the same pass, an improved feel can't be attributed to either one specifically — same one-variable-at-a-time discipline this doc has held to since 4.15's own revision note. The bug fix is a candidate for whichever pilot comes after this one, informed by what this one finds.

## Mechanism

**No new condition type, no new component.** The Collier's `known` stage (`Prototype4.unity`, currently attends-only at `minAttends:1, ripenChance:0.34`) gets the exact same treatment 4.18 gave the Netmender's: a `taughtPropertyId`, retuned to the generous once-earned taught tier (`1/0.5`), plus an `earlyTeachHintLine` and `teachLine`. Pure content, reusing `TaughtPropertyCondition`/`InOrderCondition` a third time.

**Property: `turns_water`.** The one property in `PropertyRegistry` still untouched by any P4 content (`binds_fast`/`split_prone` on the Netmender's register, `burns_slow` shared by the Smokehouse/Hearth Ring/Char Knot, `keeps_well` on the Netmender's taught stage and the Bothy's seed, `heavy_true` on the Collier's own `shares_the_watch`). Deliberately not reusing any property already gating something else in the scene — the point is a genuinely separate front, not a second door on the same key.

**Source, kept independent of the Netmender's chain on purpose:** `turns_water` added as a *second* `propertyIds` entry on **A Char Knot** (`p4_char_knot`, currently `[burns_slow]` only) in The Ash Bed's pool — mirrors A Cut Withy already carrying two properties (`binds_fast` + `split_prone`), and needs no new item or pickup. Revealed the same way every other P4 property is: worked out at the Smoking Racks, `turns_water` added to `biasPropertyIds` unconditionally (same precedent as `heavy_true`'s 4.11 addition — the Racks already reveal both sites' properties without a flag gate). The acquisition-side RNG is what's actually independent here: the Char Knot comes from **The Ash Bed**, a different `TendableSpotAttendable` instance with its own ripeness/`character` state, entirely decoupled from **The Landing**'s. Blocked at one, the player has a real second roll to make at the other — not a re-skinned version of the same wait.

**The Collier stays wary (`oneQualifyingTickPerDay: 1`) — deliberately not simplified to match the Netmender's sitting-gated shape.** Combining a taught gate with an existing daily gate on the *same* stage hasn't been tested yet (4.11's build notes flagged this exact combination as open and never exercised), and it happens to pair well with the hypothesis under test: the Collier's one-attend-per-day pacing runs off the identical `DayCycleManager.DayIndex` clock that a tending spot's ripeness bonus recovers against, so a day spent letting The Landing recatch its breath is also a day where the Collier's own gate opens back up — if that rhythm is felt at all, it's a free second data point on top of the main claim.

## Explicitly out of scope

- **The `character`/rest-timing bug** (7-26-26 finding) — held back on purpose, see above.
- **Any change to the Netmender's `binds_fast` chain**, the Osier Bed, or the Smoking Racks' existing bias entries.
- **Further daylight retuning** — settled at 7 this session, not part of this pilot.
- **Generalizing past two entities.** Still four of six entities as the pure-breadth baseline; this only widens the hard-gated set from one to two.
- **Any new tending spot, new item, or new UI.** One added property on an existing item, one bias-list entry, one stage retarget — same "purely content, reuse existing machinery" shape 4.11/4.16/4.18 all held to.

## Success criterion (playtest, not metric)

Starting from a state where both the Netmender and the Collier are Acquainted: when whichever one's RNG isn't cooperating in a given session, does turning to the *other* — a different tending spot, a different property, a different entity's gate — feel like real, decided progress, the way P2's "always something happens" value intends? Or does it just feel like walking to a second slot machine instead of the first? If the former, that's evidence 4.18's single-entity framing was itself the confound, and the fix for the vending-machine feel is "give the player more fronts," which generalizes cheaply. If the latter, the compounding-RNG diagnosis from 7-26 holds independent of content volume, and the next pilot should target the mechanism (tending's yield roll, or the ripening roll on top of it) rather than adding more parallel entities.

## Why this is the right next pilot

It's the same "nearly free" shape 4.18 itself was — one stage retarget, one added property, one bias-list entry, zero new code, reusing machinery already proven three times over (4.10, 4.11, 4.18). It isolates exactly the one variable actually named in conversation (real alternative work) from the two other levers already touched this week (daylight, settled separately; the `character` bug, deliberately held back). And it incidentally exercises an open question 4.11 flagged and never tested — a taught gate stacked on an existing daily gate — for free.

---

## Iteration 4.19 Build Notes (7-26-26)

Built and verified live in Play Mode via MCP, 0 console errors/warnings across the whole session. Zero code changed — entirely scene-content, hand-edited YAML (same standing precedent as every prior content-only pass: no `UnityEngine.Object` references touched). `maxDaylight` also confirmed persisted at `7` on this pass (the Play Mode tweak from earlier in the session hadn't been saved — fixed before this build started).

**A grammar bug caught and fixed before verification, same class as 4.11's `heavy_true` finding:** the Collier's `unfamiliarDescription` already ends with "They don't turn around." — the first draft of his new `earlyTeachHintLine` opened with the identical sentence, producing a visibly duplicated line in the overlay the moment it was checked live ("They don't turn around. They don't turn around. Whatever you know..."). Fixed by dropping the redundant opening clause; re-verified clean.

**The content:** the Collier's `known` stage gained `taughtPropertyId: turns_water`, retuned from the old attends-only `1/0.34` to the generous taught tier `1/0.5` (matching every other taught stage in the doc). `turns_water` added as a second `propertyIds` entry on **A Char Knot** (Ash Bed pool, alongside its existing `burns_slow`) and to the Smoking Racks' unconditional `biasPropertyIds`. No new item, no new pickup, no new tending spot — same minimal-footprint shape 4.18 used.

**Verified live (MCP, Play Mode, 7-26-26), one continuous real-path run:**
1. Cold load, 0 errors — Collier Unfamiliar, hint present and grammatically clean after the fix.
2. Debug-advanced to Acquainted. **The wall, confirmed with a real held attend**: `turns_water` still unknown to the player, one real Begin/Release Attend left `acquaintanceStage` at `0` — ordinary flavor visit, no ripening attempt, no error.
3. **The independent-source path, no shortcuts on the property itself**: Smokehouse debug-advanced to Known (opens the Racks, unrelated to this test); a real tending hold at **The Ash Bed** (not The Landing) eventually yielded *A Char Knot* carrying both `burns_slow` and `turns_water` unrevealed; two real held attends at the Smoking Racks revealed `burns_slow` then `turns_water` on it, the latter marked under `p3_player`. The Landing was never touched this session — confirms the two entities' acquisition RNG streams are genuinely decoupled, not just narratively distinct.
4. **Back at the Collier, unprompted**: interaction line already `"Hold E to speak of what turns water"`, hint gone, description already showing the seeded-reveal branch (`"They speak of what burns slow."` — `IsFullyAcquainted`'s same early-flip behavior 4.18 exercised on the Netmender, confirmed generalizing cleanly to a second entity).
5. A real held attend resolved as the teach tick: `knows=[turns_water, burns_slow]` on the entity immediately after, description grew in the same tick to `"They speak of what turns water, and of what burns slow."` — still `acquaintanceStage=0`.
6. **New data point past what 4.18 covered — teaching vs. the daily wary gate**: a further real held attend, same day as the teach tick, crossed to Known on its first ripening roll (`1/0.5`) — confirming the teach tick does *not* itself consume the Collier's one-qualifying-attend-per-day quota, but the very next real attend that same day both crossed *and* correctly triggered `SpentToday` (the post-cross overlay read `"...They've said what they'll say today."`). Answers the open question flagged in 4.10/4.11's build notes (whether a wary entity's daily gate should also cap the crossing tick, not just teaching) for this specific case: it does, and teaching itself sits outside it entirely — a real, previously-untested interaction, now confirmed rather than assumed.
7. **The existing post-Known chain confirmed untouched**: `Force Teach` correctly resolved to `shares_the_watch`/`heavy_true` as the next taught want (not `known` — already crossed), taught cleanly; `Advance Acquaintance` crossed to `Shares the Watch` on the existing content, `knows=[turns_water, burns_slow, heavy_true]`, description and tint matching pre-4.19 text exactly. 4.19 changed nothing about the ladder past Known.
8. 0 console errors, 0 warnings across the entire session.

**What this session did and didn't answer:** the mechanism is confirmed working — the wall holds, the two entities' RNG sources are genuinely independent (different tending spots, never touched the same pool), teaching and crossing both land correctly including the new wary/teach-priority interaction. What it didn't answer is the actual point of the pilot: whether having the Collier as a real second front changes how the wait at either one *feels*. That's the playtest this build was for — recorded here as mechanism-verified, not yet played.

---

# Iteration 4.20 — Waiting on Elsewhere: A State-Chained Known (Location Pilot)

## Where this comes from

Same 7-26-26 conversation, continued after 4.19. Sean's read on 4.19: better — no moment of "I just want this one thing but have to wait" this playthrough (even allowing for a lucky drop run) — but a slight puzzle-solving feel is creeping in, partly because "getting to know" an entity is still a dead end (no downstream effect yet — see the *Next Steps* item on whether a taught crossing needs its own payoff). Two follow-on asks: try the "second front" shape on **locations** rather than NPCs, since it's untested there; and stop reaching for the property/attribute gate every time — what else is already available to make Known cost more than Acquainted?

A survey of `IDependencyCondition` turned up two already-built, already-generic conditions never yet wired into `AcquaintableAttendable`'s own stage-gating: **`WorldStateCondition`** (checks a flag set anywhere — literally the same mechanism already powering the Smoking Racks' open/bias-bonus flags) and **`EntityStateCondition`** (checks another `DevelopableEntity`'s stage index directly, but needs a `UnityEngine.Object` reference wired by hand in the scene YAML). `WorldStateCondition` was the clear pick — no object references, and every `AcquaintanceStage` already carries a `worldStateFlag` field that gets *set* on crossing; the only gap was a matching field to *require* one.

Sean's concrete pick: **the Smokehouse's Known now depends on the Osier Bed's Known** — not an arbitrary pairing, but the missing link in a loop that already otherwise exists: the Osier Bed is wary (daily visits, 4.8); its Known crossing already sets `p4_osier_bed_known` (4.9, currently only used to extend the Racks' bias); the Smokehouse's Known already opens the Racks (4.9); the Racks already reveal the properties that gate the Netmender's own Known (4.18). One new dependency edge closes the loop end to end.

## The claim under test

Does a *state chain* — one place's Known literally depending on another place already being Known, no property or teaching involved — read as a different, valid kind of "more work" than the attribute gate, and does it carry to non-NPC entities the way 4.4 already proved plain acquaintance does? Secondary: does closing this specific loop (daily-gated Osier Bed → gated Smokehouse → Racks → gated Netmender) make the whole river-landing site read as one connected system rather than four separate tracks that happen to share a location?

## Mechanism

**One new gate shape, not a new condition type.** `WorldStateCondition` (`Mossmark.Development`) already exists and is already proven — 4.9 built the exact "another entity's crossing flips a flag, something else reacts to it" pattern this reuses, just never as a stage's own dependency before. `AcquaintanceStage` gains two fields: `requiredWorldStateFlag` (the state-chain sibling to `taughtPropertyId`) and `gateHintLine` (the state-chain sibling to `earlyTeachHintLine` — descriptive-only "not yet" texture, names nothing). `AcquaintableAttendable.Awake()`'s stage-building loop gets a third branch: when `requiredWorldStateFlag` is set, a `WorldStateCondition` is added alongside whichever base condition already applies, AND-composed via `DevelopmentStage`'s existing `params` constructor — no new composition logic.

**The hinge this surfaces, fixed before it shipped:** ripening's gate-check (`!IsFullyAcquainted || CanMakeProgress()`) only ever considered `taughtPropertyId`. A state-gated Known would have fallen through that check entirely — `IsFullyAcquainted` staying false, ripening always attempting, `TryApplyStage()` silently failing every time until the flag was set, no error but also no wall. Worse, `IsFullyAcquainted` *also* governs the seeded-knowledge overlay reveal, and unlike a taught property (independently gated by `KnowsOfSelf`), a state-gated stage's seeded properties aren't gated by anything else — so widening `IsFullyAcquainted` itself to cover this case would have leaked the Smokehouse's "The place is given to what burns slow" line at Acquainted, before the Osier Bed was even Known. Fixed by keeping `IsFullyAcquainted` exactly as it was (still taught-only, still governing the reveal correctly) and introducing a separate `NextStageIsGated` used only in the ripening check, covering both gate types plus the ladder's-end case the old formula also handled. Two members, not one widened one — deliberately, so the two concerns (when to wall, when to reveal) can't drift back into each other.

**Content:** the Smokehouse's `known` stage gains `requiredWorldStateFlag: p4_osier_bed_known` — reusing the *existing* flag the Osier Bed's Known stage already sets (4.9), not a new one — plus a new `gateHintLine`. Tuning retuned from the old attends-only `2/0.2` to the generous once-earned tier `1/0.5`, same precedent every gated stage in this doc has followed since 4.18 (once the real cost is the wait on the *other* place, the roll itself shouldn't also drag).

## Explicitly out of scope

- **Any change to the Osier Bed, the Racks, or the Netmender's existing `binds_fast` gate.** This is one new edge, not a re-plumbing of the loop.
- **A hub/aggregate gate** ("all site members Acquainted before one can go Known") — the other idea from the same conversation, a bigger claim (N conditions instead of one) that deserves its own pilot once a single chain link is understood, not bundled in here.
- **Downstream payoff for acquaintance itself.** Sean's "dead end" diagnosis is real but separate — this pilot tests whether the *gate* feels different, not whether *reaching* Known finally does something. Flagged, not fixed, here.
- **Any new condition type.** `WorldStateCondition` is reused unmodified; the only new code is the authoring hook and the ripening/reveal hinge fix above.

## Success criterion (playtest, not metric)

Working the loop for real — daily visits at the Osier Bed, the Smokehouse staying locked until it's Known, the Racks opening only then, properties feeding the Netmender's own gate — does it read as one system with a real shape, the kind of interlock 4.16 first tested with a single property edge? Or does a state-chain gate feel like the same "wait on RNG/days" texture wearing different clothes? And now that it's tested on two buildings rather than an NPC pairing, does 4.19's "second front reduces the wait-frustration" finding still hold, or was it specific to having another person to go talk to?

## Why this is the right next pilot

It's the cheapest possible way to answer "what else do we have besides the attribute gate": zero new condition types, one existing mechanism (`WorldStateCondition`) wired into a place it was never used before, and a reused flag rather than an authored new one. It also closes a loop that was already 90% built by accident across 4.8, 4.9, and 4.18 — the Osier Bed's daily gate, its already-set flag, the Smokehouse's already-existing Racks-unlock, and the Racks' already-existing feed into the Netmender were never actually chained together end to end until this edge.

---

## Iteration 4.20 Build Notes (7-26-26)

Built and verified live in Play Mode via MCP, 0 console errors/warnings across the whole session. New code confined to `Mossmark.Prototype4.AcquaintableAttendable` (two new `AcquaintanceStage` fields, a generalized `Awake()` condition-building loop, `NextStageIsGated`/`StateGateHintPending`, one more `GetOverlayDescription()` branch); no shared script touched — `WorldStateCondition` itself is reused byte-for-byte from `Mossmark.Development`. Scene change is content-only on the Smokehouse, hand-edited YAML per the standing precedent (no object references involved — `requiredWorldStateFlag` is a plain string, same as every other P4 content pass).

**The content:** Smokehouse's `known` stage gained `requiredWorldStateFlag: p4_osier_bed_known` (the Osier Bed's own Known stage already sets this flag, unchanged since 4.9) and a new `gateHintLine` ("It could be fired tomorrow and still wouldn't be right. What it's waiting on isn't you — it's upriver, and it hasn't come into its own yet."), tuning retuned `2/0.2` → `1/0.5`.

**Verified live (MCP, Play Mode, 7-26-26), one continuous real-path run:**
1. Cold load, 0 errors — Smokehouse Unfamiliar.
2. Debug-advanced to Acquainted. **The wall, confirmed with a real held attend, `p4_osier_bed_known` unset**: `acquaintanceStage` stayed at `0` — ordinary flavor visit, no ripening attempt. Overlay correctly showed `gateHintLine` appended (`StateGateHintPending` firing as designed) and, critically, **no seeded-reveal leak** — `"The place is given to what burns slow"` did *not* appear despite `burns_slow` being seeded self-knowledge from `Awake()`, confirming the `IsFullyAcquainted`/`NextStageIsGated` split holds: the wall works without the reveal leaking early, the exact failure mode the mechanism section flagged before building.
3. **The real unlock path**: Osier Bed debug-advanced to Acquainted then Known — `HandleDeveloped` logged `WorldState flag 'p4_osier_bed_known' set` (the pre-existing 4.9 behavior, untouched).
4. **Back at the Smokehouse, unprompted**: a real held attend correctly ripened (logged `"The Smokehouse: deepening (attend 1 at this depth)."`, confirming `NextStageIsGated` correctly let ripening proceed once `CanMakeProgress()` read true) but missed the first roll (`1/0.5`, a genuine coin-flip miss, not a bug); a second real held attend crossed — `"The Smokehouse: developed - Known!"`, `acquaintanceStage=1`, tint `RGBA(0.72, 0.45, 0.3, 1)` matching the pre-4.20 authored Known text exactly (confirms the destination text is unchanged, only the path to it). `HandleDeveloped` logged `WorldState flag 'p4_smokehouse_known' set`, opening the Racks exactly as before.
5. **The rest of the loop, confirmed intact**: a real tending hold at The Landing yielded A Cut Withy; the first real held attend at the now-open Smoking Racks produced no reveal and no log at all — daylight had run out (`0/7`) mid-session, the same silent `RequiresDaylight` block this doc has flagged before (4.10/4.12/4.13/4.17); `Force Rest` cleared it, and the next real held attend correctly revealed `binds_fast` on the Withy. Back at the Netmender (debug-advanced to Acquainted this session, since she wasn't touched otherwise), the interaction line auto-flipped to `"Hold E to speak of what binds fast"` unprompted, exactly as 4.18 established — confirming the full chain (Osier Bed → Smokehouse → Racks → Netmender) runs end to end through the new edge with no special-casing anywhere downstream.
6. 0 console errors, 0 warnings across the entire session.

**What this session did and didn't answer:** the mechanism is confirmed correct end to end, including the one real risk the design flagged in advance (the seeded-reveal leak) — checked for explicitly and found not to occur — and including a genuine ripening miss along the way, which is exactly what "gates can be exact, outcomes shouldn't be" predicts rather than a fluke to explain away. What it didn't answer is the actual question: whether a state-chain gate reads as a genuinely different, valid kind of "more work" than the attribute gate, whether the closed loop reads as one connected system, and whether 4.19's second-front finding holds for two locations instead of an NPC pair. Recorded here as mechanism-verified, not yet played.

---

## Playtest Findings (Sean, 7-26-26)

**Worked, but in an unexpected direction — recorded, not acted on yet.** The loop basically held together: by the time the Racks opened, the Withy had already been picked up as part of ordinary play at the site, so the whole Osier Bed loop could be completed without ever leaving it. That read as more *logical* and *directed* than 4.18/4.19's gates — though Sean flags a real confound: he already knew what was gated on what going in, so it's unclear whether blind discovery (no foreknowledge of the Osier-Bed-→-Smokehouse dependency) would feel the same way, or would read as more opaque/arbitrary without that foreknowledge.

**The trade the state chain actually made:** because the site was self-sufficient, it produced *zero* pull toward "nothing more to do here, let me go wander" — the exact feeling 4.19's second-front pilot was built to test *for*. So a state chain within one site made that site's own discovery loop feel better, but more self-contained/isolated as a side effect, which is closer to the opposite of what 4.19 was reaching for (breadth-pull across sites). Not a contradiction between the two pilots — they're pulling on different axes (one site's internal coherence vs. cross-site breadth) — but worth sitting with before choosing which axis to lean on next, rather than resolving it now.
...
- **Strategic depth-vs-breadth weight** (7-18-26, first flagged; built as Iteration 4.18 on 7-24-26, playtest pending): no real tension yet between diving deep on one site vs. spreading across several — 4.13's ripeness mechanism (7-21-26) was one candidate direction but never playtested at scene scale. 4.18 took a different, more direct swing at the same question — gating the Netmender's Known stage on an already-known property (`binds_fast`) rather than on attends alone — piloted on one entity only, with the other five left as the cheap-breadth baseline; mechanism confirmed working end-to-end live (see its Build Notes), the actual felt-pull/felt-weight success criterion not yet played. Still open regardless of how the playtest lands: whether the mechanism needs to generalize to the other entities, whether a day/commitment-based gate is needed instead of or alongside a property gate (4.18's explicit fallback if the property gate alone doesn't land), and whether the required property's source should eventually move cross-site (deliberately deferred by 4.18, and the natural next step if it does land). Whatever the fix, avoid anything with a calculable optimal order (a menu choice wearing a spreadsheet, against "Tried, not chosen"). The other organic-leaning direction still on the table: extend 4.9's cross-site bonus-bias seam (Osier Bed → Smoking Racks) so knowing several sites occasionally opens a richer read elsewhere, not as a per-visit bonus. Also worth reconnecting once picked up: Greybox's parked flow/valve thread (`WorldSite`/dominance/flow-reserve, Iterations 42–54) — the Tending Thread's yield pools are the natural upstream sources a flow system would eventually throttle.
