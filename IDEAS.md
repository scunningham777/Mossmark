...
### 7-27-26 — Debt vs. Witness (PC motivation), parked. 
Revisited the "Midlake game loop mechanics brainstorm" motivation options: Debt (PC owes something structural, discharged through understanding/small repair) vs. The World As The Goal (no personal stake — attention as a way of being, most in line with Van Occupanther's unresolved, vignette-like mood). Leaning toward the latter as the working frame — it's the cleaner fit for Tried Not Chosen, since Debt implies a ledger that eventually balances, which risks reintroducing the sequencing/arc pull P4's chaining iterations were just cut for.

Also floated: letting the tension between the two exist as a player-facing choice of focus rather than identity — not a dialogue prompt or tracked moral state, but something closer to where the player's attention habitually goes shaping how in-game development/flow balances over time. Very early, no mechanism proposed. Worth keeping distinct from the earlier version of this idea (an explicit binary choice or an inferred/tracked character-style stance), both of which risk landing as either a menu-choice-wearing-a-values-quiz or a repeat of 4.15's borderline-preachy tracked judgment. Not scoped, not touching P5.